...
def evalSymbAD(individual,toolbox,X_train,y,d):
    ...
    if d == 1:
        ...
        X_pde_train1 = torch.linspace(0, 1, 100 + 1).clone().detach().requires_grad_(True)
        pde_error1 = torch_Poisson1d(expr, X_pde_train1)
    ...
    if d == 2:
        ...
        mse = math.fsum(error) / len(error)
        X_pde_train1 = torch.linspace(0, 1, 100 + 1).clone().detach().requires_grad_(True)
        X_pde_train2 = torch.linspace(0, 1, 100 + 1).clone().detach().requires_grad_(True)
        ...
        X_pde_train1 = torch.linspace(0, 1, 100 + 1).clone().detach().requires_grad_(True)
        X_pde_train2 = torch.linspace(0, 1, 100 + 1).clone().detach().requires_grad_(True)
        X_pde_train3 = torch.linspace(0, 1, 100 + 1).clone().detach().requires_grad_(True)
        pde_error1 = torch_Poisson3d(expr, X_pde_train1, X_pde_train2, X_pde_train3)
        mse = math.fsum(error) / len(error)

    return [pde_error1+mse]

def torch_Poisson1d(expr, X1_train):
    x1 = Symbol("x1")
    expr = expr(x1)
    simplified_expr = expand(expr)
    f_torch = sympytorch.SymPyModule(expressions=[simplified_expr])
    try:
        f_x1 = f_torch(x1=X1_train)
    except:
        du_x1 = torch.zeros(X1_train.size(), dtype=X1_train.dtype)

    try:
        f_x1 = f_x1.reshape(X1_train.shape)
        # 一阶导
        du2_dx1 = torch.autograd.grad(f_x1.sum(), X1_train, create_graph=True)
    ...
    du2_dx1 = du_x1 + torch.ones(X1_train.size(), dtype=X1_train.dtype)
    mse = F.mse_loss(du2_dx1, torch.zeros_like(du2_dx1))
    #print(mse.item())
    return mse.item()
...
def torch_Poisson2d(expr, X1_train, X2_train):
    ...
    try:
        ...
        f_x1_x2 = torch.zeros(X1_train.size(), dtype=X1_train.dtype)
...
def torch_Poisson3d(expr, X1_train, X2_train, X3_train):
    ...
    try:
        ...
        f_x1_x2_x3 = torch.zeros(X1_train.size(), dtype=X1_train.dtype)
