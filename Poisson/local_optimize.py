...
def local_optimize(individual, X, y,pset):
    ...
    if len(X) == 1:
        minimize(loss1d, params,args=(individual, X, y,pset), method='SLSQP',bounds=tuple(bounds))
    elif len(X) == 2:
        minimize(loss2d, params, args=(individual, X, y, pset), method='SLSQP', bounds=tuple(bounds))
    elif len(X) == 3:
        minimize(loss3d, params, args=(individual, X, y, pset), method='SLSQP', bounds=tuple(bounds))
