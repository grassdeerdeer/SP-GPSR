...
from evaluate2 import * # 2_1D,2_2D,2_3D

def define_gp(X_train,y,d,evalSymbAD=evalSymbAD):
...
def main(d = 1):
    if d == 1:
        X_train, y_train = dataset2_1D(sample=10)
        # 2. 定义变量
        toolbox, pset = define_gp(X_train, y_train, d, evalSymbAD=evalSymbAD)
        pop = init_gp(toolbox, pset, X_train, y_train)
    elif d == 2:    
        X_train, y_train = dataset2_2D(sample=10)
        # 2. 定义变量
        toolbox, pset = define_gp(X_train, y_train, d, evalSymbAD=evalSymbAD)
        pop = init_gp(toolbox, pset, X_train, y_train)
    elif d == 3:
        X_train, y_train = dataset2_3D(sample=10)
        # 2. 定义变量
        toolbox, pset = define_gp(X_train, y_train, d, evalSymbAD=evalSymbAD)
        pop = init_gp(toolbox, pset, X_train, y_train)
